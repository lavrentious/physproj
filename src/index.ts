...
async function main() {
  ...
  document
    ...
    ?.addEventListener("click", function (event) {
      ...
      document
        .getElementById("pause-button")
        ?.children[0].classList.toggle("bi-play-fill", app.getPause());
      document
        .getElementById("pause-button")
        ?.children[0].classList.toggle("bi-pause", !app.getPause());
    });
  ...
  guiPhysicsFolder
    ...
    .name("Постоянная G");
  guiPhysicsFolder
    .add(runtimeConfig, "frictionCoef")
    .min(0.001)
    .max(1)
    .step(0.01)
    .name("Коэф. трения");
  ...
  guiSimulationFolder
    .add(runtimeConfig, "ballsNumber")
    .min(1)
    .max(15)
    .step(1)
    .name("Кол-во шаров");
  guiSimulationFolder
    .add(runtimeConfig, "placementType", PlacementType)
